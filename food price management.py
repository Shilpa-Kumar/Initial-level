...
root.title("food price management")
...
lblInfo=Label(Tops,font=('Helvetica',50,'bold','italic'),text="Food Price Management ",fg="Blue",bd=10,anchor='w')
...
def Ref():
    ...
    if (Discount.get()==""):
        ...
        CoDiscount=float(Discount.get())

                   
    CostofIdly =CoIdly * 25
    CostofCoffee=CoCoffee * 20
    CostofDosa = CoDosa* 35
    CostofKesari = CoKesari * 25
    CostPulav = CoPulav* 35
    CostKarabath=CoKarabath * 20
    CostDiscount=CoDiscount* 50

    CostofMeal= "Rs", str('%.2f' % (CostofIdly+CostofCoffee+CostofDosa+CostofKesari+CostPulav+CostKarabath))

    CostDiscount=((CostofIdly+CostofCoffee+CostofDosa+CostofKesari+CostPulav+CostKarabath)/2)

    PayTax=((CostofIdly+CostofCoffee+CostofDosa+CostofKesari+CostPulav+CostKarabath) * 0.2)

    TotalCost=((CostofIdly+CostofCoffee+CostofDosa+CostofKesari+CostPulav+CostKarabath)- CostDiscount)
 
    Ser_Charge= ((CostofIdly+CostofCoffee+CostofDosa+CostofKesari+CostPulav+CostKarabath)/99)
...
lblDiscount= Label(f1, font=('arial', 16, 'bold'),text="Discount",bd=16,anchor="w")
lblDiscount.grid(row=6, column=0)
txtDiscount=Entry(f1, font=('arial',16,'bold'),textvariable=Discount,bd=10,insertwidth=4,bg="powder blue",justify='right')
txtDiscount.grid(row=6,column=1)
